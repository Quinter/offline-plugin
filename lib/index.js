...
import defaultOptions, { AppCacheOptions as defaultAppCacheOptions, DEFAULT_AUTO_UPDATE_INTERVAL } from './default-options';

import { hasMagic, interpolateString, isAbsoluteURL, escapeRegexp, functionToString } from './misc/utils';
      ...
      const handledCaches = ['main', 'additional', 'optional'].reduce((result, key) => {
        ...
        cache.some(cacheKey => {
          ...
          if (typeof cacheKey === 'string') {
            magic = !isAbsoluteURL(cacheKey) && cacheKey[0] !== '/' && cacheKey.indexOf('./') !== 0 && hasMagic(cacheKey);
