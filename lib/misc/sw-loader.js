...
const modules = ['async-waituntil.js'];
...
module.exports.pitch = function pitch(remainingRequest, precedingRequest, data) {
  ...
  const helpersCode = [', {', `loaders: ${loadersCode},`, `cacheMaps: ${cacheMapsCode},`, `navigationPreload: ${navigationPreloadCode},`, '}'];

  Promise.all([...modules.map(mod => readFile(path.join(__dirname, mod))), readFile(templatePath).then(template => {
    template = `
        ${template}
