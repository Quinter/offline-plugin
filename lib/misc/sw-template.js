...
function WebpackServiceWorker(params, helpers) {
  ...
  function networkFirstResponse(event, urlString, cacheUrl) {
    return fetchWithPreload(event).then(response => {
      if (response.ok) {
        if (DEBUG) {
          console.log("[SW]:", `URL [${urlString}] from network`);
        }
    ...
    const storing = preloadResponsePromise.then(res => {
      ...
      return caches.open(PRELOAD_CACHE_NAME).then(cache => {
        ...
        return cache.put(url, clone).then(() => {
          if (!isSamePreload()) {
            return caches.open(PRELOAD_CACHE_NAME).then(cache => cache.delete(url));
          }
        });
      });
    });
  ...
  function retrivePreloadedResponse(event) {
    ...
    if (fromMemory) {
      event.waitUntil(caches.open(PRELOAD_CACHE_NAME).then(cache => cache.delete(request)));
  ...
  function addAllNormalized(cache, requests, options) {
    ...
    return Promise.all(requests.map(request => {
      ...
      return fetch(request, requestInit).then(fixRedirectedResponse).then(response => {
        if (!response.ok) {
