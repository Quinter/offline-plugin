...
import { getSource, pathToBase, isAbsoluteURL, isAbsolutePath, functionToString } from './misc/utils';
...
export default class ServiceWorker {
  ...
  addEntry(plugin, compilation, compiler) {
    ...
    } else if ((this.minify !== false || optimization.minimize) && makeMinificationPlugin) {
      // Do not perform auto-minification if MinificationPlugin isn't installed

      const added = (optimization.minimize && optimization.minimizer || []).concat(compiler.options.plugins || []).some(plugin => {
  ...
  apply(plugin, compilation, compiler) {
    ...
    if (typeof this.minify === 'boolean') {
      ...
      minify = makeMinificationPlugin && (!!(compiler.options.optimization && compiler.options.optimization.minimize) || !!(compiler.options.plugins || []).some(isMinificationPlugin));
    }
